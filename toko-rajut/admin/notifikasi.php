 ...
 * - Email dikirim lewat SMTP Gmail (PHPMailer), pakai SMTP_EMAIL & SMTP_APP_PASSWORD
 *   yang sudah dikonfigurasi di .env. PHPMailer di-load langsung dari libs/phpmailer/.
 * - WhatsApp: karena wa.me selalu butuh klik "Kirim" manual di WhatsApp (batasan dari
 *   WhatsApp sendiri, bukan dari kode ini), yang disediakan di sini adalah LINK yang
 *   sudah terisi pesan otomatis — admin tinggal klik sekali untuk kirim.
 */

/**
 * Template pesan per status. Placeholder {nama} dan {kode} akan diganti otomatis.
 * Status yang tidak ada di daftar ini akan pakai template default di bawah.
 */
 ...
 * (dipakai baik untuk link WhatsApp yang sudah ada maupun yang baru).
 */
function normalisasi_nomor_wa(?string $telepon): string
{
    $telepon = trim((string) $telepon);
    if ($telepon === '') return '';
    $telepon = (ltrim($telepon, '0') === $telepon) ? $telepon : '62' . ltrim($telepon, '0');
    return preg_replace('/[^0-9]/', '', $telepon);
}

/**
 * Bikin link wa.me yang sudah terisi pesan notifikasi status.
 * Return string kosong kalau nomor tidak valid.
 */
...
/**
 * Bungkus konten email dengan template branded Lalunaco (header hijau tua + footer),
 * sama gayanya dengan email yang dikirim dari kontak.php.
 */
...
/**
 * Kirim email notifikasi status ke pembeli lewat SMTP Gmail (PHPMailer),
 * pakai kredensial SMTP_EMAIL & SMTP_APP_PASSWORD yang sudah ada di .env.
 * PHPMailer di-load langsung dari libs/phpmailer/ (tidak butuh Composer/vendor/).
 * Return true kalau berhasil, false kalau gagal.
 * $errorOut diisi pesan error kalau gagal, supaya bisa ditampilkan ke admin.
 */
function kirim_email_status_pesanan(?string $emailTujuan, string $namaPembeli, string $kodePesanan, string $statusBaru, ?string &$errorOut = null): bool
{
    ...
    $smtpUser = $_ENV['SMTP_EMAIL'] ?? '';
    $smtpPass = $_ENV['SMTP_APP_PASSWORD'] ?? '';

    if ($smtpUser === '' || $smtpPass === '') {
        $errorOut = 'SMTP_EMAIL atau SMTP_APP_PASSWORD belum diisi di .env.';
        return false;
    }
    ...
    $content = "
        ...
        <table width='100%' cellpadding='0' cellspacing='0' style='margin-bottom:20px;'>
            <tr><td style='padding:6px 0;color:#786F5E;font-size:13px;width:110px;'>Kode Pesanan</td><td style='padding:6px 0;color:#2B2620;font-size:14px;font-weight:600;'>{$kodeAman}</td></tr>
            <tr><td style='padding:6px 0;color:#786F5E;font-size:13px;'>Status</td><td style='padding:6px 0;'><span style='background:#EFE3C8;color:#7C4527;padding:4px 10px;border-radius:12px;font-size:13px;font-weight:700;'>{$statusTampil}</span></td></tr>
        </table>
